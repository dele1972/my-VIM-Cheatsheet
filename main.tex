...
\command{\texttt{\%}}{move cursor to matching character}
...
%\subsection{bar 2}
\command{\texttt{<ctrl>r}}{redo}
\command{\texttt{.}}{repeat last command}
\command{\texttt{<ctrl>6}}{toggle last two Buffers}
...
\command{\texttt{n}}{repeat search in same direction}
\command{\texttt{N}}{repeat search in opposite direction}
\command{\texttt{:\%s/<old>/<new>/g}}{replace all \textit{old} with \textit{new} throughout file}
...
\command{\texttt{m<char>}}{set current position for (\textbf{m})ark \textit{char} within the file/buffer}
\command{\texttt{m<CHAR>}}{set current position for (\textbf{m})ark \textit{CHAR} global}
\command{\texttt{'<char>}}{jump to position of mark \textit{char} within the file/buffer}
\command{\texttt{m<CHAR>}}{jump to buffer and position of mark \textit{CHAR}}
